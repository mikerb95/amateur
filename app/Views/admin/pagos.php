...
$pagosCancelados = count(array_filter($usuarios, function($u) {
    // Aquí viene desde Admin::usuarios() como 'estado_pago'
                    ...
                    <form action="<?= base_url('pagos/guardar'); ?>" method="POST" class="payment-form">
                        ...
                        <div class="form-group">
                            <label for="estado" class="form-label">
                                <span class="label-icon">💳</span> Estado de Pago *
                            </label>
                            <select name="estado" id="estado" class="form-select" required>
                                <option value="Pago Pendiente"
                                    <?= (isset($pago) && ($pago['estado'] ?? '') == 'Pago Pendiente') ? 'selected' : '' ?>>
                                    ❌ Pago Pendiente
                                </option>
                                <option value="Pago Cancelado"
                                    <?= (isset($pago) && ($pago['estado'] ?? '') == 'Pago Cancelado') ? 'selected' : '' ?>>
                                    ✅ Pago Cancelado
                                </option>
                            </select>
                        ...
                        <div class="payment-summary">
                            <div class="summary-item <?= (isset($pago) && ($pago['estado'] ?? 'Pago Pendiente') == 'Pago Cancelado') ? 'paid' : 'pending' ?>">
                                <div class="summary-icon">
                                    <?= (isset($pago) && ($pago['estado'] ?? 'Pago Pendiente') == 'Pago Cancelado') ? '✅' : '❌' ?>
                                </div>
                        ...
                        <div class="form-actions">
                            <button type="submit" class="btn-submit">
                                <span class="btn-icon">💾</span> Guardar Estado de Pago
                            </button>
                            <a href="<?= base_url('pagos') ?>" class="btn-cancel">
                                <span class="btn-icon">↶</span> Buscar Otro Usuario
                            </a>
                        </div>
                    </form>
                </div>
